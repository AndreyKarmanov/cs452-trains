...
// // Configure the line properties (e.g, parity, baud rate) of a UART and
// ensure that it is enabled void uart_config_and_enable(size_t line) {
// uint32_t baud_ival, baud_fval; 	uint32_t flag = UART_LCRH_FEN;

// 	switch (line) {
// 		// setting baudrate to approx. 115246.09844 (best we can do); 1
// stop bit 		case CONSOLE: baud_ival =   26; baud_fval = 2; break;
// default: return;
// 	}

// 	// line control registers should not be changed while the UART is
// enabled, so disable it 	uint32_t cr_state = UART_REG(line, UART_CR);
...
// 	// set the line control registers: 8 bit, no parity, 1 or 2 stop bits,
// FIFOs enabled 	UART_REG(line, UART_LCRH) = UART_LCRH_WLEN_HIGH |
// UART_LCRH_WLEN_LOW | flag;

// 	// re-enable the UART; enable both transmit and receive regardless of
// previous state 	UART_REG(line, UART_CR) = cr_state | UART_CR_UARTEN |
