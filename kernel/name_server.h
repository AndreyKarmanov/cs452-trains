...
public:
  void run() {
    ...
    case MessageType::NAME_SERVER_REGISTER_AS: {
      ...
      name_to_tid.set(msg.payload.ns_register.name, sender_tid);
      reply(sender_tid, reply_msg);

      if (auto *buffer = waiting_who_is.get_ref(msg.payload.ns_register.name)) {
        auto waiting_tid_opt = buffer->pop();

        Message who_is_reply_msg{};
        who_is_reply_msg.type = MessageType::NAME_SERVER_WHO_IS_REPLY;
        who_is_reply_msg.payload.ns_who_is_reply.tid = sender_tid;
        ...
        waiting_who_is.remove(msg.payload.ns_register.name);
      }
      break;
    }
    case MessageType::NAME_SERVER_WHO_IS: {
      auto tid_opt = name_to_tid.get(msg.payload.ns_who_is.name);
      if (tid_opt.has_value()) {
        Message reply_msg{};
        reply_msg.type = MessageType::NAME_SERVER_WHO_IS_REPLY;
        reply_msg.payload.ns_who_is_reply.tid = tid_opt.value();
        reply(sender_tid, reply_msg);
      } else {
        if (auto *buffer = waiting_who_is.get_ref(msg.payload.ns_who_is.name)) {
          buffer->push(sender_tid);
        } else {
          Buffer<int, MAX_NAMES> waiting_buffer{};
          waiting_buffer.push(sender_tid);
          waiting_who_is.set(msg.payload.ns_who_is.name, waiting_buffer);
        }
      }
      break;
    }
